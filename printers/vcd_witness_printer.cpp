    ...
    const smt::Term & ast)
{
  ...
  signal_set.insert(std::make_pair(
      short_name, VCDArray(short_name, full_name, ast, data_width)));
    ...
    std::ostream & fout) const
{
  for (auto && sig_bv_ptr : allsig_bv_) {
    ...
    valbuf.insert(std::make_pair(sig_bv_ptr->hash, val));
...
void VCDWitnessPrinter::DumpValues(std::ostream & fout) const
{
  ...
  if (cex_.empty()) throw CosaException("No trace to dump");
